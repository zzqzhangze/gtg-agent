...
用 LLM 替代关键词匹配，并做精细分类：

```
task_type:
  - chat           # 纯聊天 → 直接 LLM 回复
  - compute        # 简单计算 → LLM 自己算，不进沙箱
  - code_exec      # 需要执行代码 → 进沙箱
  - data_analysis  # 数据分析 → 进沙箱 + 自动上传文件
  - multi_step     # 复杂项目 → 进沙箱 + 允许多轮迭代
```

### 方案要点

- `analyze_intent` 改为调用 LLM（few-shot prompt + 结构输出）
- 返回字段从 `needs_sandbox: bool` 扩展到 `task_type + reasoning + suggested_template`
- 根据 task_type 动态选择沙箱模板（python-sandbox / node-sandbox / data-analysis）

### 涉及文件

- `src/agent/state.py` — 扩展 intent 字段
- `src/agent/nodes.py` — 重写 `analyze_intent`
- `src/agent/graph.py` — 扩展条件路由
